 ...
 *  - MIDI task consumes ring buffer, map MIDI events to audio engine
 ******************************************************************************
 */
...
static void USB_GPIO_Init(void) {
  ...
  gpio.GPIO_Speed =
      GPIO_Speed_100MHz; /* USB FS requires fast GPIO for 12 Mbps */
  ...
  NVIC_SetPriority(OTG_FS_IRQn,
                   6U); /* numerically > 5 = below syscall ceiling */
