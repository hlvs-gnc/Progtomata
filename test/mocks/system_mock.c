...
void RCC_HSEConfig(int state) { (void)state; mock_hse_requested = 1; }
int RCC_WaitForHSEStartUp(void) { return mock_hse_ready ? 0 : 1; } /* 0 == success */
...
void RCC_PLLCmd(int state) { (void)state; mock_pll_ready = 1; }
int RCC_GetFlagStatus(int flag) { (void)flag; return 1; } /* non-zero -> ready */
void RCC_SYSCLKConfig(int src) { (void)src; }
int RCC_GetSYSCLKSource(void) { return 0x08; } /* 0x08 == PLL used as sysclk */

void RCC_PLLI2SConfig(int val, int div) { (void)val; (void)div; }
...
void GPIO_SetBits(void* a, uint32_t b) { (void)a; (void)b; }
void GPIO_ResetBits(void* a, uint32_t b) { (void)a; (void)b; }
int GPIO_ReadInputDataBit(void* a, uint32_t b) { (void)a; (void)b; return 0; }
