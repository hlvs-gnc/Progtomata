...
static USB_OTG_DevInitTypeDef usb_cfg = {.dev_endpoints = 4U,
...
static void USBD_Reset(USBD_HandleTypeDef *hdev) {
  ...
  /* Clear endpoint interrupt flags (0xFB7FU matches HAL), deactivate and NAK
   * all EPs. EP0: Just set NAK (no EPDIS per HAL), but clear USBAEP to force
   * reconfiguration. Other EPs: Disable if enabled, then NAK. */
  for (uint32_t i = 0U; i < 4U; i++) {
    ...
    if (i == 0U) {
      ...
      if (USB_OTG_FS_INEP(i)->DIEPCTL & USB_OTG_DEPCTL_EPENA) {
        USB_OTG_FS_INEP(i)->DIEPCTL =
            USB_OTG_DEPCTL_EPDIS | USB_OTG_DEPCTL_SNAK;
      } else {
...
static void USBD_RxFLVL(USBD_HandleTypeDef *hdev) {
  ...
  uint16_t bcnt =
      (uint16_t)((grxstsp & USB_OTG_GRXSTSP_BCNT) >> USB_OTG_GRXSTSP_BCNT_POS);
  uint8_t pktsts = (uint8_t)((grxstsp & USB_OTG_GRXSTSP_PKTSTS) >>
  ...
  case STS_DATA_UPDT:
    if (bcnt > 0U) {
      if (epnum < USB_OTG_FS_MAX_EP_NUM &&
        ...
        USB_OTG_FS_ReadPacket(hdev->out_ep[epnum].xfer_buff +
                                  hdev->out_ep[epnum].xfer_count,
                              bcnt);
        ...
        while (remaining > 0U) {
          uint16_t chunk =
              (remaining > sizeof(rx_drain)) ? sizeof(rx_drain) : remaining;
...
static void USBD_ProcessSetup(USBD_HandleTypeDef *hdev) {
  ...
  hdev->request.wValue =
      (uint16_t)(hdev->setup_buf[2] | ((uint16_t)hdev->setup_buf[3] << 8));
  hdev->request.wIndex =
      (uint16_t)(hdev->setup_buf[4] | ((uint16_t)hdev->setup_buf[5] << 8));
  hdev->request.wLength =
      (uint16_t)(hdev->setup_buf[6] | ((uint16_t)hdev->setup_buf[7] << 8));
  ...
  if (req_type == USB_REQ_TYPE_STANDARD) {
    ...
    /* Class / Vendor requests — stall (MIDI doesn't need class-specific EP0
     * requests) */
    USBD_CtlError(hdev);
  }
}
...
static void USBD_StdItfReq(USBD_HandleTypeDef *hdev) {
  ...
  switch (req->bRequest) {
  case USB_REQ_GET_INTERFACE:
    if ((req->wLength == 1U) && (hdev->dev_state == USBD_STATE_ADDRESSED ||
                                 hdev->dev_state == USBD_STATE_CONFIGURED)) {
...
static void USBD_StdEPReq(USBD_HandleTypeDef *hdev) {
  ...
  case USB_REQ_GET_STATUS:
    ...
    if ((hdev->dev_state == USBD_STATE_ADDRESSED ||
         hdev->dev_state == USBD_STATE_CONFIGURED) &&
