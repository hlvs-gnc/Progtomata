...
{
  if (midi_ring_is_full(rb)) { return -1; }
  uint32_t h = rb->head;
  rb->buf[h][0] = pkt[0];
  rb->buf[h][1] = pkt[1];
  rb->buf[h][2] = pkt[2];
  rb->buf[h][3] = pkt[3];
  __asm volatile ("dmb" ::: "memory");   /* Ensure data is written before head advances */
...
{
  if (midi_ring_is_empty(rb)) { return -1; }
  uint32_t t = rb->tail;
  pkt[0] = rb->buf[t][0];
  pkt[1] = rb->buf[t][1];
  pkt[2] = rb->buf[t][2];
  pkt[3] = rb->buf[t][3];
  __asm volatile ("dmb" ::: "memory");   /* Ensure data is read before tail advances */
