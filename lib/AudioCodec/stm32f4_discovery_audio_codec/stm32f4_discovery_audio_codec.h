...
// Uncomment this line to enable audio transfer with I2S interrupt
 ...
 * @{
 */
#define VOLUME_CONVERT(x)                                                      \
  ((Volume > 100) ? 100 : ((uint8_t)((Volume * 255) / 100)))
...
/* This function is called when the requested data has been completely
   transferred. In Normal mode (when  the define AUDIO_MAL_MODE_NORMAL is
   enabled) this function is called at the end of the whole audio file. In
   circular mode (when  the define AUDIO_MAL_MODE_CIRCULAR is enabled) this
...
/* This function is called when half of the requested buffer has been
   transferred This callback is useful in Circular mode only (when
   AUDIO_MAL_MODE_CIRCULAR define is enabled)*/
...
/* Codec_TIMEOUT_UserCallback() function is called whenever a timeout condition
   ...
   Typically the user implementation of this callback should reset I2C
   peripheral and re-initialize communication or in worst case reset all the
   application. */
